  ...
  */
  ...
  void run() {
    for (int i = population.size()-1; i >= 0; i--) {                       // Iterate backwards through the ArrayList in case we remove item(s) along the way
      ...
      if (generation ==3) {positions.seedpos[i] = new PVector(c.position.x, c.position.y); println("Start position updated for cell " + i);} // To update each cell's start position for the next epoch
    }
  }
}